...
describe('UserRegistrationMethodController (e2e)', () => {
  ...
  it('/user-registration-methods/:name/users (GET) --> 200 OK', () => {
    MockDataStorage.setDefaultItems();

    const initialData = [...MockDataStorage.items()];
    return request(app.getHttpServer())
      .get(`/user-registration-methods/${MockDataStorage.items()[0].name}/users`)
      .expect(HttpStatus.OK)
      .then(response => {
  ...
  it('/user-registration-methods/:name/users (GET) --> 404 NOT FOUND | User registration methods with specified name was not found', () => {
    MockDataStorage.setDefaultItems();

    const initialData = [...MockDataStorage.items()];
    return request(app.getHttpServer())
      .get(
        `/user-registration-methods/${MockDataStorage.createUserRegistrationMethodDtoList[0].name}/users`,
      )
  ...
  it('/user-registration-methods (POST) --> 409 CONFLICT | User registration methods with specified name already exists', () => {
    MockDataStorage.setDefaultItems();
  ...
  it('/user-registration-methods/:name (PUT) --> 404 NOT FOUND | User registration methods with specified name was not found', () => {
    MockDataStorage.setDefaultItems();
  ...
  it('/user-registration-methods/:name (DELETE) --> 200 OK', () => {
    ...
    const initialData = [...MockDataStorage.items()];
    console.log(MockDataStorage.removeUserRegistrationMethodDtoList);
    return request(app.getHttpServer())
      .delete(
  ...
  it('/user-registration-methods/:name (DELETE) --> 404 NOT FOUND | User registration methods with specified name was not found', () => {
    MockDataStorage.setDefaultItems();
